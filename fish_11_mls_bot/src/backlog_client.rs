...
impl BacklogClient {
    ...
    pub async fn connect(&self) -> Result<mpsc::Sender<BacklogMessage>> {
        ...
        let stream = TcpStream::connect(&self.addr).await
            .map_err(|e| anyhow::anyhow!("Failed to connect to backlog peer {}: {}", self.addr, e))?;

        let peer_addr = stream.peer_addr().ok();
        info!("Connected to backlog peer {} ({})", self.addr,
                ...
                tokio::select! {
                    Some(msg) = rx.recv() => {
                        match serde_json::to_vec(&msg) {
                            Ok(json) => {
                                let len = (json.len() as u32).to_be_bytes();
                                if let Err(e) = tokio::io::AsyncWriteExt::write_all(&mut writer, &len).await {
                                    error!("Failed to write to backlog peer {}: {}", addr_clone, e);
                                    break;
                                }
                                if let Err(e) = tokio::io::AsyncWriteExt::write_all(&mut writer, &json).await {
