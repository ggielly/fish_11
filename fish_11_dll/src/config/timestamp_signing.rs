...
pub fn verify_timestamp(signed_timestamp: &str, signing_key: &[u8; 32]) -> Result<String> {
    ...
    let delimiter_pos = signed_timestamp.rfind(SIGNED_TIMESTAMP_DELIMITER).ok_or_else(|| {
        FishError::CryptoError("Invalid signed timestamp format: missing delimiter".to_string())
    })?;
    ...
    mac.verify_slice(&signature_array).map_err(|_| FishError::AuthenticationFailed)?;
...
pub fn extract_timestamp(signed_timestamp: &str) -> Result<String> {
    if is_signed_timestamp(signed_timestamp) {
        let pos = signed_timestamp.rfind(SIGNED_TIMESTAMP_DELIMITER).unwrap();
